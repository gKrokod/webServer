...
import Database.Esqueleto.Experimental (insertMany_, insert_)
...
-- import Database.Migration (migrate0, MigrateTable(..))
import Database.Migration (MigrateTable(..), MyMigration(..), migratePlan)
...
dropAll = rawExecute "DROP TABLE IF EXISTS news, images_bank, images, categories, users, passwords, migrate_table" []

migrationStart :: ConnectionString -> IO ()
-- prodymat' proverky v database pered apply.
migrationStart pginfo = do
  putStrLn "Migration0"
  runDataBaseWithOutLog pginfo $ do 
    mapM_ applyMigrate migratePlan
      where applyMigrate (MkMigration v d c) = do
              runMigration c
              time <- liftIO getCurrentTime
              insert_  (MigrateTable {migrateTableVersion = v, migrateTableDescription = d, migrateTableTime = time})
