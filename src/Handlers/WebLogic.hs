...
endPointNews h req = do
  ...
  case rawPathInfo req of
    ...
    "/news" -> do -- get all news
      ...
      mapM_ (Handlers.Logger.logMessage logHandle Handlers.Logger.Debug . T.pack . show) 
      ...
      case checkCopyright of
        Right Valid -> do
          ...
          tryEditNews <- Handlers.Base.updateNews baseHandle 
            title_ newTitle_ newLogin_
            newLabel_ newContent_ (fromMaybe [] newImages_) newIsPublish_
          case tryEditNews of
            Right _ -> do
