...
import Types (Label (..), Login (..), NumberCategory (..), NumberImage (..), NumberNews (..), PasswordUser (..), Title (..))
...
data Handle m = Handle
  { logger :: Handlers.Logger.Handle m,
    ...
    findCategoryById :: NumberCategory -> m (Either SomeException (Maybe Category)),
    ...
    editCategoryId :: CategoryInternalId -> m (Either SomeException Success)
  }
