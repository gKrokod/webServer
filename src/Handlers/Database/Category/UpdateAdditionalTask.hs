...
updateCategoryBaseAdditionalTask :: (Monad m) => Handle m -> Label -> CategoryInternal -> m (Either T.Text Success)
updateCategoryBaseAdditionalTask h label newCat@(CategoryInternal newlabel parent) = do
